		...
		if (postData.typeOfAsset) 
			{ 
				if (!postData.typeOfAsset.includes('topic') && !postData.typeOfAsset.includes('post')){
					try { 
						await db.decrAssetcount(`user:${postData.uid}`, postData.typeOfAsset); 
						await db.decrAssetTime(`user:${postData.uid}`, postData.typeOfAsset, postData.timestamp - postData.startedAt); 
